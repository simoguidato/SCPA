...
PALETTE = {
    ...
    'CUDA_Opt2D':   '#c0396b',
    'CUDA_Tiled':   '#f08030',
    'CUDA_WarpRow': '#9b59b6', # Viola per WarpRow
    'CPU_Best':     '#27ae60',
}
LABELS = {
...
# Ora include Opt2D, Tiled E WarpRow!
modes_to_plot = ['CUDA_Opt2D', 'CUDA_Tiled', 'CUDA_WarpRow']
if has_cpu:
    modes_to_plot = ['CPU_Best'] + modes_to_plot

df_k32_gpu = df_gpu_mean[(df_gpu_mean['k'] == 32) & (df_gpu_mean['Mode'].isin(['CUDA_Opt2D', 'CUDA_Tiled', 'CUDA_WarpRow']))].copy()
if has_cpu:
...
ax.set_ylim(0, df_k32['GFLOPS'].max() * 1.25)
...
for idx, size in enumerate(SIZES_4):
    ...
    for mode in ['CUDA_Opt2D', 'CUDA_Tiled', 'CUDA_WarpRow']:
    ...
    # Speedup dinamico: Calcola max GPU vs CPU
    if has_cpu:
        gpu_max_k32 = df_gpu_mean[(df_gpu_mean['Size_Label']==size) &
                                  (df_gpu_mean['k']==32) &
                                  (df_gpu_mean['Mode'] != 'CUDA_Naive')]['GFLOPS'].max()
        cpu_k32 = best_cpu[(best_cpu['Size_Label']==size) & (best_cpu['k']==32)]['GFLOPS']

        if not np.isnan(gpu_max_k32) and not cpu_k32.empty:
            ratio = gpu_max_k32 / cpu_k32.values[0]
            ax.text(0.97, 0.05, f'MAX GPU/CPU = {ratio:.1f}×', ha='right', va='bottom',
